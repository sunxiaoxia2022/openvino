...
void Engine::ApplyPerformanceHints(std::map<std::string, std::string> &config, const std::shared_ptr<ngraph::Function>& ngraphFunc) const {
    ...
    if (mode_name == CONFIG_VALUE(LATENCY)) {
        ...
        auto get_num_streams = [&](bool aggressive) {
            const auto num_cores = getNumberOfCPUCores();
            ...
            return engConfig.streamExecutorConfig._big_core_streams + engConfig.streamExecutorConfig._small_core_streams;
        };
        ...
        if (networkToleranceForLowCache.max_mem_tolerance == ov::MemBandwidthPressure::UNKNOWN) {
            if ((networkToleranceForLowCache.ratio_compute_convs == ov::MemBandwidthPressure::ALL)
                ...
                num_streams = get_num_streams(true);
            ...
            num_streams = get_num_streams(false);
        }
